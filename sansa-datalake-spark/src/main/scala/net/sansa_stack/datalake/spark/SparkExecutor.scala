...
class SparkExecutor(spark: SparkSession, mappingsFile: String) extends QueryExecutor[DataFrame] {
    ...
    def query (sources : Set[(HashMap[String, String], String, String)],
        ...
        for (s <- sources) {
            ...
            println("sourcePathsourcePath: " + sourcePath)
        ...
        /******* THIS IS JUST FOR TEST - Uncomment when needed *******/
        // println("Number of Spark executors: " + spark.sparkContext.statusTracker.getExecutorInfos.length)
        // println("Master URI: " + spark.sparkContext.master)
