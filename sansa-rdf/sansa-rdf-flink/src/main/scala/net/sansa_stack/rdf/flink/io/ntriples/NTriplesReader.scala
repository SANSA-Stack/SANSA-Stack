...
/**
  ...
  */
object NTriplesReader {

  /**
    * Loads N-Triples data from a file or directory into a DataSet.
    *
  ...
  /**
    * Loads N-Triples data from a set of files or directories into a DataSet.
...
//            ErrorHandlerFactory.errorHandlerWarn
  ...
  def main(args: Array[String]): Unit = {
    if (args.length == 0) println("Usage: NTriplesReader <PATH_TO_FILE>")

    val path = args(0)

    val env = ExecutionEnvironment.getExecutionEnvironment

    val ds = NTriplesReader.load(env,
      path,
