  ...
  def assessLiteralNumericRangeChecker(triples: DataSet[Triple]): Long = {
    ...
    val s2 = triples.filter(f =>
      f.getPredicate.matches(RDF.`type`.asNode())
        && f.getSubject.matches(NodeFactory.createURI(subject)))

    val s = s2.filter(f => f.getPredicate.matches(NodeFactory.createURI(property))
      && (f.getObject.getLiteralValue.toString().toDouble > lowerBound && f.getObject.getLiteralValue.toString().toDouble < upperBound))

    val S = s.distinct(_.hashCode()).count()
    val S2 = s2.distinct(_.hashCode()).count()
