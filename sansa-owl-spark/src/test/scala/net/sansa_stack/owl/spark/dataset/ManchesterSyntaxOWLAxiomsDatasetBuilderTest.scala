...
class ManchesterSyntaxOWLAxiomsDatasetBuilderTest extends FunSuite with SharedSparkContext {
  ...
  ignore("Annotation assertion axioms should be created correctly") {
    ...
    implicit val encoder = Encoders.kryo[OWLAnnotationAssertionAxiom]
    val filteredDataset: Dataset[OWLAnnotationAssertionAxiom] =
      dataset.filter(_.isInstanceOf[OWLAnnotationAssertionAxiom]).map(
        _.getAxiomWithoutAnnotations[OWLAnnotationAssertionAxiom])

    assert(filteredDataset.count() == expectedNumberOfAxioms)

    val annAxiom = filteredDataset.first()
  ...
  ignore("Has-key axioms should be created correctly") {
    ...
    val dataPropExprs =
      filteredDataset.first().getAxiomWithoutAnnotations[OWLHasKeyAxiom].dataPropertyExpressions()
