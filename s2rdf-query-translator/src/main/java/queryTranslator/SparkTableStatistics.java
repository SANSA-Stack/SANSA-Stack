...
//			  System.out.println("table name(" + temp[0] + ") = " + tableName);
	...
	private static long readExtVPStatistic(String relType){
		...
		try
		{
		  ...
		  while ((line = reader.readLine()) != null && !line.contains("-------------")){
			  ...
			  String tableName = tableName(temp[0]);
			  if (relType.equals("SO")) soStats.put(tableName, newStat);
			  else if (relType.equals("OS")) osStats.put(tableName, newStat);
			  else if (relType.equals("SS")) ssStats.put(tableName, newStat);
		  }
		  reader.close();
		}
		catch (Exception e)
	...
	public static String tableName(String predicate) {
		return predicate.replace(":", "__").replace("<<", "<").replace(">>", ">").replace("/", "%2F");
	}
