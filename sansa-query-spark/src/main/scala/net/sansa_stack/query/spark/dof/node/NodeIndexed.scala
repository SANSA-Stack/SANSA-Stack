...
class NodeIndexed[N: ClassTag](data: RDD[N]) extends java.io.Serializable {
  ...
  // RDD[Node].distinct looses a lot of entity when whe use Node as base type for RDD
  ...
  // There is no such a problem with loosing nodes on distinct()
  // when we use String as base type for RDD
  ...
  def print: Unit = rddStr.foreach(d => Helper.log(d.toString))
}
