...
/**
  ...
  */
object clusteringRDFByModularity {

  def apply(numIterations :Int, graphFile :String, outputFile : String) = {

    // DEFAULT INPUT
    // val (numIterations, graphFile) = (100 , "C:/Users/hpetzka/IdeaProjects/Clustering_in_Spark/Graphs/testRDF.txt")

    // SPARK CONTEXT AND INITIAL RDDs
    val config = new SparkConf().setMaster("local[*]").setAppName("Clustering")
