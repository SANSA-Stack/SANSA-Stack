...
//    +-----+-----+
    ...
    processPrediction(rdfFastTreeGraphKernel, 4, algorithm, iteration)
  }
  ...
  def experimentMultiContractPrediction(sparkSession: SparkSession, depth: Int, algorithm: Int, iteration: Int): Unit = {
    ...
    processPrediction(rdfFastTreeGraphKernel, 2, algorithm, iteration)
  }
...
//    +-----+-----+
  ...
  def processPrediction(rdfFastTreeGraphKernel: RDFFastTreeGraphKernel, numClasses: Int = 2, algorithm:Int = 1, iteration: Int = 2): Unit = {
    ...
    if (algorithm == 1) {
      ...
      val data = rdfFastTreeGraphKernel.getMLFeatureVectors
      data.cache()
...
//    predictions.foreach(println(_))
  ...
  def predictRandomForestML(data: DataFrame, numFold: Int = 1): Unit = {
  ...
  def predictLogisticRegressionML(data: DataFrame, numFold: Int = 1): Unit = {
