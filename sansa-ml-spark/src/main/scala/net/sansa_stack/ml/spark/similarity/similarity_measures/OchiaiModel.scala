...
class OchiaiModel extends GenericSimilarityEstimatorModel {
  ...
  override val estimator_name: String = "OchiaiSimilarityEstimator"
  override val estimator_measure_type: String = "similarity"

  override val similarityEstimation = ochiai

  override def similarityJoin(df_A: DataFrame, df_B: DataFrame, threshold: Double = -1.0, value_column: String = "ochiai_similarity"): DataFrame = {
  ...
  override def nearestNeighbors(df_A: DataFrame, key: Vector, k: Int, key_uri: String = "unknown", value_column: String = "ochiai_similarity", keep_key_uri_column: Boolean = false): DataFrame = {
