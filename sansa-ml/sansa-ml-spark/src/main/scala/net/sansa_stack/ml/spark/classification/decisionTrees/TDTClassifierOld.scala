...
object TDTClassifierOld {
  ...
  class TDTClassifiers(var kb: KB, var sc: SparkSession) extends Serializable {

   var nodes: Int = 0
   val reasoner: StructuralReasoner = new StructuralReasoner(kb)
    ...
    def induceDLTree(father: OWLClassExpression,
      ...
      if (posExs.count == 0 && negExs.count == 0) // There is no examples
        if (prPos >= prNeg) { // prior majority of positives
          tree.setRoot(kb.getDataFactory.getOWLThing) // set positive leaf
          nodes += 1
          println("\nPositive leaf (prior1)")
          return tree
        } else { // prior majority of negatives
          tree.setRoot(kb.getDataFactory.getOWLNothing) // set negative leaf
          nodes += 1
      ...
      val Con: RDD[OWLClassExpression] = generateRefs(kb, father, nRefs, posExs, negExs)
//      Con.take(50).foreach(println(_))

      // select best partitioning node concept
      val bestConcept: OWLClassExpression = selectBestConcept(kb, Con, posExs, negExs, undExs, prPos, prNeg)
    ...
    def classify(ind: OWLIndividual, tree: DLTree): Int = {
      ...
      val negRootClass: OWLClassExpression = kb.getDataFactory.getOWLObjectComplementOf(rootClass)

      if (rootClass.equals(kb.getDataFactory.getOWLThing)) return 1
      if (rootClass.equals(kb.getDataFactory.getOWLNothing)) return -1

      var r1: Int = 0
      var r2: Int = 0
      var result: Int = 0

      if (reasoner.isEntailed(kb.getDataFactory.getOWLClassAssertionAxiom(rootClass, ind))) {
        r1 = classify(ind, tree.getPosSubTree)
      }
      else if (reasoner.isEntailed(kb.getDataFactory.getOWLClassAssertionAxiom(negRootClass, ind))) {
    ...
    //  val ConceptExp = concept.asConjunctSet().iterator().asScala.toSeq
      ...
      for (c <- 0 until dim) {
        ...
        do {
          ...
          val Concepts = new HashSet[OWLClassExpression]()

          if (concept.isOWLThing) {
            refinement = new RefinementOperator(kb).getRandomConcept(kb)
          }
          else {
...
//      println("\nNo. of generated concepts: " + nRef)
      Refs.distinct()
          ...
          /* val con: OWLEquivalentClassesAxiom = kb.dataFactory.getOWLEquivalentClassesAxiom(concept)
...
//              newConcept = kb.getDataFactory.getOWLObjectIntersectionOf(Concepts)
//              if (newConcept != null) {
//                emptyIntersection = !kb.getReasoner.isSatisfiable(newConcept)
//              }
//              else emptyIntersection = false
//            }
//          }
    ...
    // val iterator: Iterator[OWLIndividual] = kb.getReasoner().getInstances(newConcept, false).entities().iterator().asInstanceOf[Iterator[OWLIndividual]]
    ...
    def selectBestConcept(kb: KB,
      ...
      var counts: Array[Int] = getSplitCounts(kb, concepts.first(), posExs, negExs, undExs)
      ...
      println("\n#0" + " \tPL:" +counts(0) +",\t NL:" + counts(1) + ",\t UL:" + counts(2) + ",\tPR:" + counts(3) +
        ",\tNR:" + counts(4) + ",\tUR:" + counts(5) + ",\t gain: " + f"$bestGain%8.3f")

      val noOfConcepts = concepts.count.toInt
      for (c <- 1 until noOfConcepts) {

        val nConcept = concepts.take(noOfConcepts).apply(c)
      ...
      val nCpt = concepts.take(noOfConcepts).apply(bestConceptIndex)
      ...
      if (bestGain == 0)  {
        ...
        println("\n --------\nBest gain: " + f"$bestGain%8.3f " + " \t Split index: " + bestConceptIndex
    ...
    /**
      * @param counts, array contaning the number of +ve, -ve and uncertain examples for each branch
    ...
    private def splitGroup(kb: KB,
      ...
      val negConcept: OWLClassExpression = kb.getDataFactory.getOWLObjectComplementOf(concept)
  
      val leftRDD = nodeExamples.filter { indv =>
        val nodeInd = kb.getDataFactory.getOWLNamedIndividual(indv.asOWLNamedIndividual())
        reasoner.isEntailed(kb.getDataFactory.getOWLClassAssertionAxiom(concept, nodeInd))
      }
  
      val rightRDD = nodeExamples.filter { indv =>
        val nodeInd = kb.getDataFactory.getOWLNamedIndividual(indv.asOWLNamedIndividual())
        reasoner.isEntailed(kb.getDataFactory.getOWLClassAssertionAxiom(negConcept, nodeInd))
      }
...
//    */
  ...
  /* def selectBestConceptEntropy(kb: KB, concepts: RDD[OWLClassExpression],
                                posExs: RDD[String],
                                negExs: RDD[String],
                                undExs: RDD[String],
...
//        val nodeInd = kb.getDataFactory.getOWLNamedIndividual(nodeEx)
//
//        if (kb.getReasoner.isEntailed(kb.getDataFactory.getOWLClassAssertionAxiom(concept, nodeInd))) {
//          Left.add(nodeEx)
//
//        } else if (kb.getReasoner.isEntailed(kb.getDataFactory.getOWLClassAssertionAxiom(negConcept, nodeInd))) {
//          Right.add(nodeEx)
//
//        } else {
//          Left.add(nodeEx)
...
// val propName: RDD[String] = kb.getIndividuals().map( ind => ind.asOWLNamedIndividual().getIRI.getShortForm)
//  println("\n nodeEx = " + nodeEx )
// val Filtered = kb.getIndividuals().filter(_ == nodeInd)
// println("\n filtered = " )
// Filtered.take(10).foreach(println(_))
// val exIndex = ex.lookup(e)
// println("the element: ")
// exInd.take(1).foreach(println(_))
// val ind  = kb.getDataFactory().getOWLNamedIndividual(IRI.create(nodeEx)).asInstanceOf[OWLIndividual]
// println("newexample  " + ind )

// val x = kb.getIndividuals().take(nodeExamples.count.toInt).apply(e)
// val x = kb.getIndividuals().filter( _ == neew).first()

// x.take(20).foreach(println(_))

// val r =kb.getReasoner().isEntailed(kb.getDataFactory.getOWLClassAssertionAxiom(concept, ind))
// println("\n r = " + r)

// val l =kb.getReasoner().isEntailed(kb.getDataFactory.getOWLClassAssertionAxiom(negConcept, ind))
// println("\n l = " + l)