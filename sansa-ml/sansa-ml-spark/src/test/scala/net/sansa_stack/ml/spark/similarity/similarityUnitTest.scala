  ...
  /*
    ...
    test("Test Batet Similarity Module") {
      ...
      val tmpCvDf: DataFrame = cvModel
        .transform(filteredFeaturesDataFrame)
        .cache()
      val isNoneZeroVector = udf({ v: Vector => v.numNonzeros > 0 }, DataTypes.BooleanType)
      val countVectorizedFeaturesDataFrame: DataFrame = tmpCvDf
        .filter(isNoneZeroVector(col("vectorizedFeatures")))
      ...
      // Batet Similarity
      val model = new BatetModel()
        .setInputCol("vectorizedFeatures")
      model.nearestNeighbors(countVectorizedFeaturesDataFrame, sample_key, 10).count()
      model.similarityJoin(countVectorizedFeaturesDataFrame, countVectorizedFeaturesDataFrame, threshold = 0.5).count()
    }
